...
def test_1_graph_info():
    ...
    assert gskos_i == pg.GraphInfo(144, 252, {'type', 'definition', 'range', 'title',
                                              'unionOf', 'example', 'seeAlso', 'label',
                                              'comment', 'creator', 'rest', 'domain',
                                              'scopeNote', 'contributor', 'description',
                                              'inverseOf', 'first', 'subPropertyOf', 'disjointWith',
                                              'subClassOf', 'isDefinedBy'})
...
def test_3_two_cycles_graph():
    assert pg.graph_info(pg.create_two_cycles_graph(42, 29, ('f', 's'))) == \
